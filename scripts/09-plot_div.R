...
data <- "E:/Lectures_Bern/Sem_3/eukaryotic_annotation/ERR11437321_hifiasm.fa.mod.out.landscape.Div.Rname.tab"
...
ggsave(filename = "E:/Lectures_Bern/Sem_3/eukaryotic_annotation/plot_div_output.pdf", width = 10, height = 5, useDingbats = F)
...
rep_table.m <- rep_table.m %>%
  mutate(
    age_years = distance / (2 * r),
    age_Myr   = age_years / 1e6    # in million years
  )

library(scales)
ggplot(rep_table.m, aes(fill = fam, x = distance, weight = value / 1e6)) +
  geom_bar() +
  cowplot::theme_cowplot() +
  scale_fill_brewer(palette = "Paired") +
  scale_x_continuous(
    name = "Divergence (proportion)",
    sec.axis = sec_axis(~ . / (2 * r) / 1e6, name = "Estimated insertion age (Myr)")
  ) +
  ylab("Sequence (Mbp)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, size = 9, hjust = 1),
        plot.title = element_text(hjust = 0.5))
ggsave(filename = "E:/Lectures_Bern/Sem_3/eukaryotic_annotation/output_with_age.pdf", width = 10, height = 5, useDingbats = F)
